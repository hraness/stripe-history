...
export const privacyDescription =
  `How ${site.domain} handles analytics, cookies, the Hraness Substack embed, historical mailing consent, and hosting logs for the independent Stripe company history.`;
...
export const privacyParagraphs = [
  ...
  `The footer loads the canonical Hraness subscription embed from [Substack](https://hraness.substack.com/). Loading it sends Substack the ordinary request and delivery metadata needed to serve the frame. If you submit an email address, Substack handles that address, confirmation, Hraness newsletter delivery, and unsubscribe under its own service and privacy terms.`,
  `Stripe History does not send new footer subscriptions to Hraness Accounts and does not create new Stripe History mailing consent. If you subscribed through the earlier Stripe History form, Hraness Accounts retains that dated record. A previously confirmed membership may remain active, and Hraness Accounts may continue to process it and deliver Stripe History newsletter messages through Resend from news.hraness.com until you use its Stripe-History-specific unsubscribe link. That link does not change another product or general Hraness subscription. Removing the earlier form does not delete, cancel, or migrate that record.`,
...
export const contactParagraphs = [
  ...
  `There is no Stripe History-owned reader login, contact form, product inbox, or new product-specific mailing signup on ${site.domain}. The footer carries the canonical Hraness Substack embed described on the privacy page. The project does not process payments, issue API keys, or operate a Stripe integration. ${independenceSentence}`,
  `Published and maintained by [Hraness](${HRANESS_URL}). The complete sourced records and website code are in the [Stripe History repository](${GITHUB_REPOSITORY_URL}). Read [about](${SITE_ORIGIN}/about) for editorial method and [privacy](${SITE_ORIGIN}/privacy) for analytics, the Substack embed, and historical mailing-consent limits.`,
] as const;
