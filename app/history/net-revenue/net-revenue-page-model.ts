...
function formatNameList(names: readonly string[]): string {
  if (names.length <= 1) return names[0] ?? "its source";
...
export function deriveNetRevenuePageSeo(
  ...
  const latestPublishers = [
    ...new Set(latestRecord.event.sources.map(({ publisher }) => publisher)),
  ];
  ...
  return {
    ...
    lead: [
      `Stripe’s latest full-year company figure is ${latestRecord.point.display} in ${latestKind} for ${latestRecord.point.calendarYear}, according to ${formatNameList(latestPublishers)}.`,
      records.length === 1
        ? `This page shows the one year with a sourced figure.`
        : `This page shows the ${countWord(records.length)} years with a sourced figure between ${firstRecord.point.calendarYear} and ${latestRecord.point.calendarYear}.`,
      "Stripe publishes no glossary that defines net revenue. This page uses the definition Forbes reported in May 2022: what Stripe keeps after the cut passed to partners such as Visa and Chase.",
      "That makes it a different measure from payment volume, the 2.9%+30¢ list price, a take rate, free cash flow, and net income.",
      ...(hasBothKinds
    ...
    method: [
      ...
      "The first Stripe-authored use of net revenue as a company KPI is the leaked August 19, 2026, investor letter, which gives first-half growth rates and no dollar total.",
      "The working definition comes from Forbes on May 26, 2022, citing people who had seen the books: “Net revenue, which excludes the cut Stripe passes along to partners like Visa and Chase.” Stripe declined to comment and has not contradicted that definition.",
      ...
      "The Wall Street Journal’s April 13, 2021, figure of about $7.4 billion for 2020 “Revenue” is later treated as gross and is not plotted.",
      ...
      "Product-level “$100 million of net revenue” in the August 2026 investor letter is the same metric on a product line and is not a company full-year point.",
