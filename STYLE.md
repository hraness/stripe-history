...
- End an event summary on its last sourced fact. A sentence about what an event meant for Stripe or its founders needs a source that says so, with attribution.
- Detail rows state facts about the event. Do not publish notes about how this timeline records, dates, classifies, or plots an event, and do not point out spelling mistakes in a source. The displayed date already shows its precision.
- Write status as a short lowercase phrase for the state of the event, such as announced, agreement reported, completed, or launched. Keep dates, caveats, and notes out of status.
- Keep deal states distinct: reported talks, a reported agreement, an announced agreement, and a completed transaction.
- Label each annual figure with the term its source uses, such as payment volume or total volume, revenue or net revenue.
- Prompts that write timeline or appearance text start with the shared block in `lib/generation-style.ts`.
